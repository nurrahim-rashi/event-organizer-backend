...
export const createTransactionService = async (body, userId) => {
    ...
    return await prisma.$transaction(async (tx) => {
        ...
        if (activeTransaction) {
            throw new ApiError("You have an ongoing transaction. Please finish payment or cancel it first.", 400);
        }
        let totalPrice = 0;
        ...
        for (const item of items) {
            ...
            if (!ticket || ticket.totalTicket - ticket.booked < item.qty) {
                throw new ApiError(`Ticket ${ticket?.name || "unknown"} is out of stock`, 400);
            }
...
export const acceptOrRejectTransactionService = async (transactionId, status) => {
    return await prisma.$transaction(async (tx) => {
...
export const uploadPaymentService = async (transactionId, userId, paymentProof) => {
...
export const cancelTransactionService = async (transactionId, userId) => {
    return await prisma.$transaction(async (tx) => {
        const transaction = await tx.transaction.findUnique({
            where: { id: transactionId },
            include: { items: true },
...
export const getActiveTransactionService = async (userId) => {
...
export const getIncomingTransactionService = async (eventId, userId) => {
...
export const updateTransactionStatusService = async (transactionId, status) => {
...
export const getTransactionsByEventService = async (eventId) => {

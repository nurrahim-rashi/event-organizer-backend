...
export const getVoucherById = async (req, res) => {
    const voucher = await voucherService.getVoucherByIdService(Number(req.params.id));
    res.status(200).json({ success: true, data: voucher });
};
export const updateVoucher = async (req, res) => {
    const voucher = await voucherService.updateVoucherService(Number(req.params.id), req.body);
    res.status(200).json({ success: true, data: voucher });
};
